// Runtime configuration for Cloudflare Pages
// This approach fetches config at runtime instead of build time
window.APP_CONFIG = {
  VITE_SUPABASE_URL: "https://tbcodjhsyktoldyzflax.supabase.co",
  VITE_SUPABASE_ANON_KEY:
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRiY29kamhzeWt0b2xkeXpmbGF4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjA3MjcyMjcsImV4cCI6MjA3NjMwMzIyN30.rD6ICOoQSmUCpAdUp8QnXnHMO3jwyiEXThoXM2bdDgY",
  VITE_MAPBOX_TOKEN:
    "pk.eyJ1IjoiZGptYXJ0aW4yMDE5IiwiYSI6ImNtZ3doNjdheDA4YWcya29jdWluamJiOHIifQ.6GnpfyZAEN4PL5zBXOsP_A",
};
